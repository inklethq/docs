...
- Type tables in the SDK reference are generated from the published
  `@inklethq/sdk` declaration files pinned in `package.json`. `<SdkType />` is
  only used for types the pinned release still describes correctly; anything
  newer is written out as a TypeScript block in the page until the release
  carrying it is on npm and the pin is moved.
...
3. Replace the hand-written TypeScript blocks for those types with
   `<SdkType />`, and add any new source file to the `file` union in
   `components/mdx.tsx`.
4. Update `contracts/public-api.yaml` for HTTP-level changes.
5. Update the relevant guide in `content/` when behavior or workflows change.
6. Run `pnpm check` and verify both the SDK and HTTP reference pages locally.
