...
| --- | --- | --- |
| `AuthenticationFailedError` | The service rejected the token | Check that it is valid and active |
...
| `PermissionDeniedError`, code `access_denied` | Valid token, but that resource is not the owner's | Check the id you passed |
| `PermissionDeniedError`, code `plan_upgrade_required` | Valid token, but the feature needs Pro | Manage the subscription in the inklet portal; keep the same PAT |

All of these extend `InkletError`; the first three also extend
