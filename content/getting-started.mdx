...
console.log(result.contentId, result.analysisId, result.state);
```

### Wait for it to land

The call above returns as soon as inklet has your assets and has accepted the
Analysis — usually with `state: "queued"` and no Presentation IDs yet. Wait for
the run to finish:

```ts
const analysis = await inklet.analyses.wait(result.analysisId);

if (analysis.outcome === "no_change") {
  console.log("nothing worth showing:", analysis.noChangeReason);
} else {
  console.log("presentations:", analysis.presentationIds);
}
```

A `failed` Analysis throws `AnalysisFailedError`. To watch the run instead of
waiting on it, see [Analysis events](/api/events).
...
async function main() {
  ...
  console.log(`analysis ${result.analysisId} (${result.state})`);
}
...
- [Plans and AI features](/plans) — Free vs. Pro, quotas, and history depth
- [Pushing content](/push) — the differences between Auto, Manual, and Hardcode
- [Lifecycle](/lifecycle) — states, idempotency, and knowing when a frame is real
- [Analyses](/api/analyses) — the full surface behind `push.*`
