...
<SdkType file="analyses" name="AnalyzeInput" />
...
`analyze()` is never retried automatically. If it fails — a timeout, a dropped
connection, a `5xx` — the Analysis may already exist, so call it again with
`error.idempotencyKey`, the key it sent (generated when you passed none), rather
than a fresh one.
...
<SdkType file="analyses" name="DirectInput" />
...
<SdkType file="analyses" name="ListAnalysesOptions" />
...
| `timeoutMs` | `120000` | 1–1,800,000 — a hard deadline for the whole wait, including a read in flight |
| `signal` | — | Aborting throws `OperationAbortedError`, and cancels a read in flight |

A state this SDK does not know is not treated as final. Up to three failed
reads in a row — a dropped connection, a timeout, or HTTP 408, 429, 500, 502,
503, or 504 — are retried with back-off before the error is thrown.
...
<SdkType file="analyses" name="Analysis" />
...
<SdkType file="analyses" name="AnalysisScope" />
