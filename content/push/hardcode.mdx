...
```ts
inklet.push.hardcode(input: HardcodePushInput, options?: CallOptions): Promise<HardcodePushResult>
```

<SdkType file="push" name="HardcodePushInput" />

Note that `image` is a single asset, not an array — the shape enforces the rule.
There is no `intent` or `context`: nothing is being reasoned about.

Underneath, this is [`contents.upload()`](/api/contents#upload) followed by
[`analyses.direct()`](/api/analyses#direct), which is the no-AI path.
